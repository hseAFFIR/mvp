...
class EnglishStemmer:
        ...
        for suffix in suffixes:
            if word.endswith(suffix):
                ...
                if (
                    len(word) > 2
                    and word[-1] in ["y", "Y"]
                    and word[-2] not in self.english_vowels
                ):  # Step 1c required here since it is unreachable otherwise
                    word = word[:-1] + "i"
                return word
            ...
            if word.endswith(suffix):
                if any(vowel in word[: -len(suffix)] for vowel in self.english_vowels):
                    ...
                    if word.endswith(("at", "bl", "iz")):
                        ...
                        len(word) > 1
                        and word[-2:] not in self.english_doubles
                        and self.english_is_short(syllable, R1)
                    ):
                        word += "e"
                    break
    ...
    def english_is_short_syllable(self, word):
        if (
            len(word) == 2
            and word[0] in self.english_vowels
            and word[1] not in self.english_vowels
        ):
            return True
        if len(word) < 3:
            return False
        if (
            word[-3] not in self.english_vowels  # Non-vowel before
            and word[-2] in self.english_vowels  # Vowel
            and word[-1] not in self.english_vowels
            and word[-1] not in ["w", "x", "Y"]  # Non-vowel, not 'w', 'x', or 'Y'
        ):
            return True
        return False
                ...
                if word.endswith(suffix):
                    if word[len(word) - len(suffix) :] in R1:
                        if suffix == "ogi" and len(word) > 3 and word[-4] == "l":
                            ...
                            suffix == "li"
                            and len(word) > 2
                            and word[-3] not in self.english_valid_li_endings
                        ):
                            break
                        else:
                            word = word[: -len(suffix)] + replacement
    ...
    def step5(self, word, R1, R2):
        # Flags so there's no accidental trimming (ex. versaille => versaill =>
        # versail shouldn't happen)
        e_flag = False
        l_flag = False
        if word.endswith("e"):
            e_flag = True
    ...
    def english_stemmer(self, word: str) -> str:
        ...
        word = "".join(
            "Y" if i > 0 and word[i] == "y" and word[i - 1] in self.english_vowels else c
